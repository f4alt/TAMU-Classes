...
void Component::draw(const std::shared_ptr<Program> prog,
  ...
  glm::vec4 light_dir = MV->topMatrix() * glm::vec4(lights_vec[0], 1.0);
  lights_vec[0] = glm::vec3(light_dir);
     ...
     MV->pushMatrix();
       ...
       MV->pushMatrix();
           ...
           if (HUD) {
             glUniform3fv(prog->getUniform("lightInfo"), 2, glm::value_ptr(lights_vec[2]));
           } else {
             glUniform3fv(prog->getUniform("lightInfo"), 2, glm::value_ptr(lights_vec[0]));
           }
